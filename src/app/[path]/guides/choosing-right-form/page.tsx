...
export async function generateMetadata({
  ...
  const jurisdictionName =
    (pathToServerConfig[params.path] as { name: string })?.name || 'your state';
  return {
    title: `How to Select the Right Court Form - ${jurisdictionName} - Court Forms Online`,
    description:
...
export default async function SelectRightCourtForm({ params }: PageProps) {
  const jurisdictionPath = params.path.toLowerCase();
  const jurisdictionName =
    (pathToServerConfig[jurisdictionPath] as { name: string })?.name ||
    'your state';

  // Get legal help info for the jurisdiction
  ...
  return (
    <div className={styles.AboutPageContainer + ' container'}>
      ...
      {deepLink && DisclaimerComponent && (
        <div className="mt-4 p-4 bg-light rounded">
          <h3>Get legal help in {jurisdictionName}</h3>
          <p>
            If you need additional help, connect with legal aid organizations
            and other resources available in {jurisdictionName}.
          </p>
          <a
            href={deepLink}
            target="_blank"
